...
"""
Bridge ROS 2 topics and services to the warehouse robot dashboard.

Bridges ROS 2 topics and services to the browser-based robot operations
dashboard over WebSocket and REST, using FastAPI and uvicorn running in
a background thread alongside the node's normal rclpy spin loop.

This node is monitoring and control UI only. It has no authority over
/cmd_vel and holds no safety state of its own. It mirrors state already
published by the safety and health nodes and forwards operator requests
through the same ROS 2 services used by other clients.
"""
